    ...
    if (wifiagindex >= 0) {
        if (WiFi.SSID(wifiagindex) == SSID_nameXX) {
         // i = 1000;
          ...
          Serial.print("Belirtilen ağ ");
          Serial.print(SSID_nameXX);
          Serial.println(" şu anda bulunamadı. 5 dk içinde yeniden aranacak");
      }
    }//if (WiFi.status() != WL_CONNECTED) {
    else {
      MQTTbaglan();
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  String mqtt_msg = msgIN;
  Serial.println(mqtt_msg);

  //Serial.print("payload[01]: ");
  //Serial.println( payload[0]);

  if (mqtt_msg == "power" ) {
    Serial.println("power geldi");
    ...
    //val = 0;
      ...
      uint16_t rawDatac[23] = {972, 806,  1864, 808,  976, 806,  972, 806,  974, 808,  972, 1696,  972, 808,  972, 808,  972, 810,  1862, 808,  970, 810,  972};  // RC5 78
      irsend.sendRaw(rawDatac,23,38);        
  } else if(mqtt_msg == "pageup" ){
